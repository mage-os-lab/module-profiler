...
/**
 * Arms the profiler for the current request: from the environment, the MAGE_PROFILER cookie, the
 * X-Mage-Profiler header or the flag file.
 *
 * Runs from bootstrap.php before the ObjectManager exists, so the request abstractions are unavailable
 * and superglobals, include and filesystem functions are the only option here. A FrankenPHP worker
 * runs it again for every request (Plugin\App\WorkerRequest): the arming at worker boot saw no request.
 */
...
{
    /**
     * Area flags promoted from the request into the environment; unset again by disarm().
     *
     * @var string[]
     */
    private static $promoted = [];

    /**
     * Whether this process is a FrankenPHP worker that serves many requests.
     *
     * @return bool
     */
    public static function isWorker(): bool
    {
        return function_exists('frankenphp_handle_request')
            && (getenv('FRANKENPHP_WORKER_ENABLE') ?: ($_SERVER['FRANKENPHP_WORKER_ENABLE'] ?? '')) !== '';
    }

    /**
     * Close the report of the request and return the process to its unarmed state.
     *
     * @return void
     */
    ...
    public static function arm(): void
    {
        ...
        if ($isHtmlRequest && !self::isWorker()) {
            /*
